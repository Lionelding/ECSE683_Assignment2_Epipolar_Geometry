...
load F1.txt; load F2.txt;
m1 = F1; m2 = F2;

s = length(m1);
m1=[m1(:,1) m1(:,2) ones(s,1)];
m2=[m2(:,1) m2(:,2) ones(s,1)];
Width = 506; %image width
Height = 379; %image height

% Intrinsic Matrix
load intrinsic_matrixF.txt
K = intrinsic_matrixF;
...
imageSize = [379 506];
[isIn_left,epipole_left] = isEpipoleInImage(F,imageSize);
[isIn_right,epipole_right] = isEpipoleInImage(F',imageSize);
